...
when they are the whole memory. A credential-shaped string (detected via the shared `b12_pii_scrubber` patterns) is **never boosted** — it is held at baseline so secrets are not amplified or resurfaced (redaction itself is the scrubber's job, which runs earlier on every write path). Regex scans are bounded with linear (non-backtracking) patterns so a large pasted blob cannot stall the synchronous store. This value flows into `metadata.importance_score` and the effective-stability aging below; the read-side normalization is unchanged (see RET-3).
