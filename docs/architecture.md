...
[docs/DESIGN-Phase2-Importance.md](DESIGN-Phase2-Importance.md)). A credential-shaped string (detected via the shared `b12_pii_scrubber` patterns) is **held at baseline** by the scorer, so writers that store its output unmodified (`memory_store`, `write_time_merge`) never amplify or resurface secrets. Writers that set their own `importance_score` (the checkpoint floor, PreCompact, the CLI store) bypass the cap — importance only, since the value is scrubbed — and the OpenCode plugin native path scrubs nothing (a potential raw-secret leak). The robust fix is a centralized `finalize_importance()` helper; all of this is documented as follow-ups in the [design doc](DESIGN-Phase2-Importance.md). Regex scans are bounded with linear (non-backtracking) patterns so a large pasted blob cannot stall the synchronous store. This value flows into `metadata.importance_score` and the effective-stability aging below; the read-side normalization is unchanged (see RET-3).
