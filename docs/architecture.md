...
[docs/DESIGN-Phase2-Importance.md](DESIGN-Phase2-Importance.md)). A credential-shaped string (detected via the shared `b12_pii_scrubber` patterns) is **held at baseline** on the Python write paths so secrets are not amplified or resurfaced (redaction itself is the scrubber's job, which runs earlier on those paths). Exceptions (documented as follow-ups in the [design doc](DESIGN-Phase2-Importance.md)): the checkpoint `max(score, 0.7)` floor and the PreCompact hard-coded `importance_score = 1.5` (importance only — the value is still scrubbed), and the OpenCode plugin native write path (which scrubs nothing — a potential raw-secret leak). Regex scans are bounded with linear (non-backtracking) patterns so a large pasted blob cannot stall the synchronous store. This value flows into `metadata.importance_score` and the effective-stability aging below; the read-side normalization is unchanged (see RET-3).
