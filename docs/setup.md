...
| Continue.dev | **Automatic** | Reload the VS Code/JetBrains window that hosts Continue. | Open the Continue panel → **MCP**; `B12` must be enabled and list its tools. | `~/.continue/settings.json` or `~/.claude/settings.json` should contain the B12 lifecycle hook commands; the installer accepts either hook source. |
