...
— a single chokepoint that returns baseline for a secret (overriding the type
floor, the heuristic, and any supplied value) and otherwise the strongest of the
heuristic score, the `memory_type` floor, and the supplied value. The five
**generic-content** Python writers route through it fully: MCP `memory_store`,
`write_time_merge` (insert + merge), the checkpoint hook (`supplied=0.70`), the
PreCompact hook (`supplied=1.5`, replacing the hard-coded `1.5`), and the CLI
