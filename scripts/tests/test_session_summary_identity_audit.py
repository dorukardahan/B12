...
def test_read_only_audit_classifies_every_active_unbound_summary(tmp_path: Path) -> None:
    ...
    _add(
        conn,
        17,
        {"source_session": "fedcbafedcba"},
        "proj:unique-prefix,session-summary",
    )
    _add(
        conn,
        18,
        {"session_id": "fedcbafedcba-only-full-id"},
        "proj:other",
        memory_type="decision",
    )
    conn.commit()
    ...
    assert report["active_session_summaries"] == 12
    assert report["unbound_session_summaries"] == 10
    assert report["category_counts"] == {
        "ambiguous_legacy": 6,
        "bound": 2,
        "intentionally_unbound": 1,
        "recoverable_legacy": 3,
    }
    assert [row["id"] for row in report["unbound_rows"]] == [*range(3, 11), 16, 17]
    ...
    assert by_id[17]["category"] == "recoverable_legacy"
    assert by_id[17]["recovery_source"] == "metadata.source_session"
