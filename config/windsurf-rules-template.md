...
These tools are provided by the B12 MCP server. Cascade exposes them automatically when the server is connected.
...
memory_search(query="deployment issues", after="2025-01-10", before="2025-01-15")

# Rate a memory's quality
memory_quality(action="rate", content_hash="abc123", rating="1", feedback="Very useful, referenced 3 times")
```
