...
| Tool | Purpose |
|------|---------|
| `memory_store` | Store decisions, patterns, errors, preferences to persistent memory |
| `memory_search` | Search memories by keywords, tags, dates, or semantic similarity |
| `memory_update` | Update tags, metadata, or strength of existing memories |
| `memory_quality` | Rate memory quality, get stats, or analyze system health |
...
```
memory_session_context(project_name="<project>")
```
...
Cline uses XML syntax to invoke MCP tools:
...
<use_mcp_tool>
  <server_name>B12</server_name>
  <tool_name>memory_search</tool_name>
  <arguments>{"query": "recent architecture decisions", "tags": ["proj:myproject"]}</arguments>
</use_mcp_tool>
...
<use_mcp_tool>
  <server_name>B12</server_name>
  <tool_name>memory_search</tool_name>
  <arguments>{"query": "deployment issues", "after": "2025-01-10", "before": "2025-01-15"}</arguments>
</use_mcp_tool>

<!-- Rate a memory's quality -->
<use_mcp_tool>
  <server_name>B12</server_name>
  <tool_name>memory_quality</tool_name>
  <arguments>{"action": "rate", "content_hash": "abc123", "rating": "1", "feedback": "Very useful, referenced 3 times"}</arguments>
</use_mcp_tool>
```
