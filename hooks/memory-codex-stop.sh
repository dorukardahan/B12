...
#
# fd-detach note: do NOT wrap the body in `{ ... } 2>>"$ERR_LOG"`. A brace
# group with a redirect makes bash save the original fd 2 to a high-numbered
# fd (10+) so it can restore on group exit, and that save-fd is INHERITED
# by the background subshell we fork below. Since the subshell's
# `</dev/null >>"$SESS_LOG" 2>&1` only rebinds fds 0/1/2, the save-fd keeps
# holding Codex's stderr pipe — and Codex's `subprocess.communicate()`
# blocks on stderr-EOF until the 120s sleeper exits. Using `exec 2>>` makes
# the redirect permanent (no save-fd) so the subshell inherits nothing
# pointing back at Codex.
...
# Permanent stderr redirect via `exec` (see fd-detach note above). Must come
# after mkdir so ERR_LOG's parent exists.
...
# turn-end is never blocked. The subshell's `</dev/null >>"$SESS_LOG" 2>&1`
# rebinds fds 0/1/2 so Codex's `subprocess.communicate()` sees EOF on its
# stdout/stderr pipes immediately and the turn-end isn't held open for the
# full 120s sleep.
